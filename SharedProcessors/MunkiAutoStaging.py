...
class MunkiAutoStaging(Processor):
    ...
    input_variables = {
        ...
        "NAME": {
            "description": (
                "Name of the Munki item to be checked."
            ),
            "required": True,
        },
    ...
    def _find_matching_item(self, repo_library, name):
    ...
    def _find_pkginfo_files_in_repo(self, pkginfo, file_extension="plist"):
        ...
        file_list = glob.iglob(os.path.join(destination_path, "**", f"{pkginfo_basename}*{file_extension}"), recursive=True)
    ...
    def _find_items_to_promote(self, repo_library):
        ...
        for item in items:
            ...
            files = self._find_pkginfo_files_in_repo(item, self.env["MUNKI_PKGINFO_FILE_EXTENSION"])
            ...
            for file in files:
                ...
                if ("catalogs" not in pkginfo) or (self.env["MUNKI_TESTING_CATALOG"] not in pkginfo["catalogs"]):
                    continue
            
                if ("_metadata" not in pkginfo) or ("creation_date" not in pkginfo["_metadata"]):
                    continue
